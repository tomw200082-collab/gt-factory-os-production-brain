...
def recipes():
    d = card("drinks/recipes.yaml")
    rows = d["recipes"]
    prices = {r["name"]: r for r in card("drinks/catalog.yaml")["drinks"]}
    out = ["# מתכוני 48 המשקאות\n",
           stamp("מה נכנס לכל משקה, ואיך מגישים אותו."), "",
    ...
    for cat in ("תה", "צ'אי", "אבקות"):
        ...
        out += [f"## {cat} · {len(g)} משקאות", ""]
        for r in g:
            pr = prices.get(r["name"], {})
            head = f"### {r['name']}"
            if pr:
                head += f"  ·  ₪{pr['cost']:g} → ₪{pr['price']:g}"
            out += [head, ""]
            doses = " + ".join(f"{x['item']} ₪{x['cost']:g}" for x in r["doses"])
            out += [f"**מנות:** {doses}  =  **₪{r['approved_cost']:g}**", ""]
            if r.get("serve"):
                out += [f"**הגשה:** {' · '.join(r['serve'])}", ""]
            if r.get("rounding_note"):
                out += [f"> {r['rounding_note']}", ""]
    return "\n".join(out)
...
def readme():
    ...
    return "\n".join([
        ...
        "| 03 | מוצרים ומחירים | המספרים — ומה נכנס לכל משקה |",
        ...
        f"- **{len(card('drinks/recipes.yaml')['recipes'])}** מתכונים — מנות מאושרות "
        "+ שיטת הגשה (`03 · מתכונים`)", "",
        "## איזו אמת יושבת איפה", "",
        "| נושא | האמת | מי מעדכן |",
        "|---|---|---|",
