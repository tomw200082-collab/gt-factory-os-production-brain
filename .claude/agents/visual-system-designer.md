...
- A **visual / layout** finding MUST cite a screenshot path under `UX_SHOT_OUT`.
  (This is your primary dimension — render desktop and mobile and judge the pixels.)
- A **structural / flow / copy / a11y-semantic** finding cites `file:line` instead
  (no forced screenshot where it adds nothing).
- Per-surface API data is supplied via `UX_SHOT_FIXTURE` (a JSON file), not by
  editing the harness. Auth is the dev-shim only — never `X-Fake-Session` /
  `X-Test-Session`, never a Supabase production login in CI.
...
- severity `P0` (blocks correct/confident operator action) · `P1` (harder, not
  impossible) · `P2` (polish). Map your existing finding class onto this.
