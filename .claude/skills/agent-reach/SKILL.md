...
> **GT note (added on adoption — not upstream).** This skill is a routing table
> for a command-line tool that is **not installed by default**. Before following
> anything below, check: `agent-reach doctor --json`. If the command is not found,
> the tool is not installed here — say so and stop, rather than guessing commands.
> Install is `pip install agent-reach` plus per-platform setup; in the remote
> Claude Code container that install is blocked, and the login-backed platforms
> (Instagram, Facebook, LinkedIn, Reddit, XiaoHongShu) need a desktop Chrome
> session that no container has. See `.claude/skills/VENDORED.md` for what works
> where, and for the account-risk note before reading any logged-in platform.
