...
3. כתוב אוטונומית דרך Google Calendar MCP, לפי המוסכמה הנעולה של
   `docs/ceo/reference/luz_rules.md`: כותרת בקידומת `GT · `, שורה ראשונה
   בגוף `[cos-os] type=<deep|batch|task|rock> week=<YYYY-MM-DD>`,
   `colorId` לפי הסוג, `Asia/Jerusalem`. **חוק הבעלות (משם): עדכון/מחיקה
   רק לאירועים שגופם מכיל `[cos-os]`** — אירוע בלי הסמן = של טום או של
   אחר, ⊥ לגעת. (הכרעת טום 2026-08-06, ספק §2.4+§2.7 — האוטונומיה חדשה;
   מיקום הסמן ופורמט הבלוק לפי `luz_rules.md` הנעול 2026-08-01.)
...
לילה ריק ⇒ שורה מפורשת "אין תוצרים הלילה" — ⊥ סעיפים ריקים בשקט.
