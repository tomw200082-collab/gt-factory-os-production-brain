...
- Stock Truth Layering Change 4 — Shopify available-for-sale write spec. SPEC ONLY — `SHOPIFY_BLIND_AVAILABLE_WRITE_ENABLED` remains `false`. PR #27 merged to `gt-factory-os:main` (`61700d4`) 2026-05-14. Three spec docs in `docs/integrations/`: `shopify_available_write_contract.md` (19 sections, 9 approval gates AW-G1..AW-G9, 10 UNRESOLVED items), `shopify_available_dry_run_evidence_pack.md`, `shopify_available_handler_skeleton.md`. Core formula: `available = GREATEST(0, calculated_on_hand) − committed_qty` (open LionWheel orders). Flag flip requires Tom written approval + all AW-G1..AW-G9 gates + dry-run PASS + ≥24h soak + RUNTIME_READY signal.
