...
**Deferred items (need Tom decision later, NOT blocking Run F):**
...
- **RUNTIME_READY signal count: 36** (re-verified 2026-07-17 direct from `.claude/state/runtime_ready.json`, the sole authority — was 17 → 18 ProductionActual-FromPlan → 19 CriticalToday → 20 AddFromRecommendations → 21 Planning-Tranche2-RecommendationDetail-v1.1 → 22 ProductionPlanSlippage → 23 DashboardCriticalToday → 24 DashboardSlippedPlans → 25 AdminHolidays → 26 ForecastSeedCells → 27 HolidaysArchivedFilter → 28 LionWheelOrderNote → 29 LionWheelCreditDecisionBackend → 30 PlannedInflowByDay → 31 LionWheelRuntimeClosure → **32 ProductionActual-TwoHead** (Tom-screenshotted critical bug fix; 6 tranches landed 2026-05-02; verifier PASS; **FULL GO 2026-05-02** — Tom confirmed visual portal acceptance on FG-DES-1L; data-fix follow-on migration 0130 closed the 5 MAR/SAN `pack_no_base_bom_line` anomaly via version-bump path; see `PRODUCTION/docs/two_head_bom_repair_evidence.md` + `PRODUCTION/docs/2026-05-02-two-head-bom-explosion-repair-plan.md`) → 33 InventoryFlowPlannedInflowEndpoint → 34 Forecast-Monthly → 35 GoodsReceipt-FromPO → **36 UnitEconomics** (2026-07-15T08:04:02Z, backend-db-executor, Decision Board true-gross-profit corridor — see this file's "Procurement corridor" section above for adjacent 2026-07 work). Prior "35" narrative mislabeled #26 as `HolidaysArchivedFilter`, omitting `ForecastSeedCells` — corrected here; the count itself was right at the time, only the per-signal labels below it were off by one.
...
> **Stale-calibration note (2026-07-17, source-of-truth-auditor + main-session verification):** This range (`~60-70%`) was last set in the 2026-04-23 calibration and has never been updated since — not by the 2026-05-23 post-cutover pass (which only flagged it as D3 "NEEDS_TOM_CALIBRATION" and suggested 80-90%, see below), and not by this pass either, since **completion-range is Tom-set by design, not auto-computed.** Evidence gathered as of 2026-07-17 continues to support recalibrating upward: portal readiness scorecard 92/100 (`gt-factory-os-portal/docs/portal-os/scorecard.md`, `last_reviewed: 2026-07-03`, 9/10 categories ≥8); LionWheel bridge in continuous daily production use since 2026-05-10 (781 `FG_OUT_PICK` rows in the 14 days before this check); `rebuild_verifier() = 0` (live-checked 2026-07-17, no drift); ~155 commits landed across `gt-factory-os` + `gt-factory-os-portal` in the 2026-05-18→2026-07-17 window spanning economics, production-planning, order-intake, decision-board, and procurement corridors (see this file's "Procurement corridor" section above for the most recent slice, first-hand verified). This paragraph only stamps the staleness and the supporting evidence — Tom still sets the number.
...
**W2 mode (authoritative source: `.claude/state/active_mode.json`):** W2 current mode = **A** as of 2026-05-02T22:00:00Z (re-verified 2026-07-17 direct from the JSON — corrects a stale 2026-04-27T08:18:00Z timestamp this file previously carried). Most recent Mode B exit: B-Planning-Corridor / `inventory-flow-planned-inflow-overlay` (cycle 21, atomic commit `f439cb4`, exited via reconciliation Path A). No Mode B entries recorded in `active_mode.json` history since. Earlier Mode B exits (pre-2026-05-02, preserved as history): Planning-Tranche3-Blockers (commit `e7dce27`, 2026-04-27); Planning-Tranche2-RecommendationDrillDown (1184bcf + fix 2146b84, 2026-04-26); InventoryFlow (f4aca4f + dfd2c6b, 2026-04-26); PurchaseOrders-manual (47de463 → merged `92efbb3`, 2026-04-26); PurchaseOrders-po-list-action (48720d1, 2026-04-25); Portal-Refactor Tranches A–E + Ralph Loop corridors (2026-04-21–25). Per `EXECUTION_POLICY.md`, only one form may be in Mode B at a time.
...
- **Count-freeze-driven LionWheel reversal class — CLOSED 2026-05-23, re-verified 2026-07-17.** 29 rows on 2026-05-13 use idem-key format `reversal-YYYY-MM-DD-<SKU>` and reverse "items pre-picked before morning count" (count-freeze interaction). Distinct from the delivery-correction reversal class. Ratified in `LOCKED_DECISIONS.md` §LionWheel line 140 (Tom direct edit) — text confirmed present on disk, no action outstanding. The underlying race also now has an operational fix (see "Residual risks" §11c above, migration 0277).

- **`LIONWHEEL_PICK_ADJUSTMENT` movement_type — CLOSED 2026-05-23 (Tom decision Option A), re-verified 2026-07-17.** 6 Tom-approved manual backfill rows posted 2026-05-17 are accepted in place. Ratified in `LOCKED_DECISIONS.md` §LionWheel line 144 — text confirmed present on disk. Production code may not emit it; any future use requires Tom approval. Historical rows were NOT rewritten (by design — rewrite would need a separate Tom-approved data-correction migration; none has been requested).

- **`audit_runs` daily cron not writing (P1 monitoring blind-spot — CONFIRMED STILL OPEN 2026-07-17)** — table created by migration 0151 (21 columns); `row_count = 0` re-confirmed via live read-only query 2026-07-17 (was also 0 on 2026-05-23). No commit in `gt-factory-os` since 2026-05-18 touches `JOB_RUNNER_TOKEN`, `AUDIT_SCRIPT_PATH`, or an audit-skill container. Root cause unchanged: `JOB_RUNNER_TOKEN` provisioning + Python audit-skill container deployment, both still Tom-pending from the original cutover runbook §"Open Tom dependencies". The blind spot has now persisted through ~10 weeks of live production traffic (781 `FG_OUT_PICK` rows in the 14 days immediately before this check alone) with zero remediation activity — no longer a fresh gap, an aged one. Read-only diagnosis plan: `TEST-GT-START/docs/ruflo/26_PROPOSED_BRAIN_PATCH_DIFF.md` §5.
...
- **Shopify External Boundary v2 Gate E — STALLED, plus a governance anomaly flagged 2026-07-17 (needs Tom ruling — see "Tom decisions" list below this file's header area is not it; tracked here and in `ACTIVE_NOW.md`).** GE-1 test SKU = `ADD-GAR-ANISE`; GE-2 sentinel strategy = Option C (SKU-allowlist), decided 2026-05-23. No backend or portal commit advances the corridor since `bcb2d0f` (2026-05-08) — confirmed by `source-of-truth-auditor` grep across all `gt-factory-os` commits back to 2026-06-26 (earliest reachable in this clone) plus this file's own prior "unchanged" notes. **Code-level hard stop confirmed intact 2026-07-17** (main-session direct read of `gt-factory-os/supabase/functions/factory_os_jobs/index.ts` lines 87-90, 2933-3039): `SHOPIFY_FG_SYNC_LIVE_ADAPTER_WIRED = false` is hardcoded, gates every real GraphQL mutation (`if (live && SHOPIFY_FG_SYNC_LIVE_ADAPTER_WIRED && isMutatingIntent)`), and the code comment states tests `api/test/shopify_policy.test.ts` R1 suite S21-S24 prove the guard "unbypassable." Empirically confirmed live 2026-07-17: `shopify_fg_sync_history` in the last 7 days shows 81,312 rows `disabled_pending_v2`, 2,688 `skipped_unmapped`, 1,344 `network_fail`, **zero `ok`** — no live writes are occurring. `SHOPIFY_BLIND_AVAILABLE_WRITE_ENABLED` remains `false`.
  **Anomaly:** a separate live DB table, `private_core.feature_flags`, has a row `shopify_fg_sync_v2_live` with `enabled=true`, `value={"allowlist":"*"}`, `updated_by="claude-session-2026-06-20"`, `updated_at=2026-06-20T09:20:09Z`. No migration file in `gt-factory-os/db/migrations/` corresponds to this write (checked). The `"*"` (all-SKU) scope contradicts the recorded GE-2 decision (`ADD-GAR-ANISE` only, Option C). Current code does not read this flag anywhere (grep-confirmed) — it appears to be inert prep for a future PR (the flag's own description references "commit d3f1f10", which does not exist in this repo's reachable history, so likely unmerged/planned). **Practical risk: none currently** (the code-level sentinel above is the actual gate and is separately intact and tested). **Governance risk: real** — an untracked, unmigrated production DB write with a scope value that doesn't match what was decided. **Needs Tom's ruling**, not a silent fix: was this write authorized; was `"*"` intentional; revert, tighten to `ADD-GAR-ANISE`, or ratify as a deliberate pre-Phase-5 holding state; and a migration should capture whatever the settled value is, for traceability going forward.
...
11. **LionWheel pick-reconciliation chain — OPERATIONAL POST-CUTOVER** (supersedes pre-cutover risk text 2026-04-30..2026-05-08). Bridge flipped Sunday 2026-05-10. Live ledger evidence audited 2026-05-23: 487 `FG_OUT_PICK` rows (2026-04-30 → 2026-05-21); 138 distinct LW tasks reconciled; `rebuild_verifier() = 0` confirms parity holds post-cutover. Matcha carton/bag production-code defect fixed in backend commit `b12e230` (2026-05-17); 6 historical rows were repaired via Tom-approved manual `LIONWHEEL_PICK_ADJUSTMENT` backfill 2026-05-17 (idem prefix `lw_fg_out_pick_x22_backfill:`); per Tom decision 2026-05-23 (Option A) those rows are accepted in place and the value is ratified for Tom-approved-manual-only use, with production-code emission still forbidden and any future use requiring Tom approval. Count-freeze interaction reversals (29 rows on 2026-05-13, idem prefix `reversal-YYYY-MM-DD-<SKU>`) are an operationally legitimate second reversal class — see `LOCKED_DECISIONS.md` §LionWheel amendments (Tom direct edit, same PR). Pre-cutover root-cause report `docs/lionwheel_chain_root_cause_2026-04-30.md` and repair plan `docs/lionwheel_chain_repair_plan_2026-04-30.md` are historical reference only; the chain is in operation.
    ...
    **Residual risks (post-cutover, not blocking but worth tracking):**
    a) `audit_runs` table empty — see UNRESOLVED item "`audit_runs` daily cron not writing" above (confirmed still open 2026-07-17, ~10 weeks with no remediation).
    b) **CLOSED** — `LOCKED_DECISIONS.md` §LionWheel lines 140/144 confirmed (re-verified 2026-07-17) to carry both ratifications: the count-freeze interaction reversal class, and `LIONWHEEL_PICK_ADJUSTMENT` as Tom-approved-manual-only (Option A). No text gap remains.
    c) Two reversal classes coexist (delivery-correction vs count-freeze-driven) — the root cause of the count-freeze class (Thursday count vs. same-day picks racing) now has a systematic operational fix: migration `0277_fg_out_movement_pause.sql` (backend commit `dcb9a99c`, 2026-07-03) added an admin/planner-flippable "pause LionWheel FG-out decrements" toggle, with portal UI in tranche 118 (`gt-factory-os-portal/docs/portal-os/tranches/118-movement-log-fg-out-pause.md`). The 29 historical 2026-05-13 reversals remain as-is (informational); recurrence is now operator-controlled instead of silent.
    d) **CLOSED** — signal #31 `LionWheelRuntimeClosure` (2026-05-02T14:00:00Z, executor-w1) is present in `.claude/state/runtime_ready.json`, re-verified 2026-07-17.
