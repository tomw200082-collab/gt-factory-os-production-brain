...
- Stock Truth Layering Change 1 — display clamp + Reconcile badge + StockTruthDrawer on `/inventory` (2026-05-13). Backend additive (`on_hand_raw` / `on_hand_display` / `is_below_floor` / `floor_gap` on `/api/v1/queries/stock`; `b160ffd` on `gt-factory-os:stock-truth-display-clamp`). Portal primitives + inventory migration + Playwright smoke (`948b42c`, `395f77c`, `d67ed08`, `222fd56`, `ebe5935`, `3e244d1` on `window2-portal-sandbox:stock-truth-display-clamp`). Truth surfaces (rebuild_verifier, planning inputs, audit, ledger) untouched. Other surface migrations (dashboard, item detail, form previews, movement-log) deferred to follow-up plan. Spec: `docs/superpowers/specs/2026-05-13-display-clamp-physical-stock-truth-design.md`. Plan: `docs/superpowers/plans/2026-05-13-display-clamp-physical-stock-truth.md`. Awaiting Tom merge (no autonomous push).
