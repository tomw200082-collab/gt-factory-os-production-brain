...
- **Cycle 8 partial state (rate-limit interrupt 2026-05-02)**: W1 archived_at consumer filter migration `db/migrations/0120_holidays_il_archived_filter.sql` + pgTAP `db/tests/0120_*.test.sql` + checkpoint `docs/holidays_il_archived_filter_checkpoint.md` + 4 inspection scripts ALL ON DISK UNCOMMITTED on canonical main. Signal #26 `RUNTIME_READY(HolidaysArchivedFilter)` ALREADY EMITTED to harness `runtime_ready.json`. Migration application to live DB UNCONFIRMED — Tom or W1 (post rate-limit reset 3:40am Asia/Jerusalem) must verify + commit + push. W2 `/admin/holidays` page changes (page.tsx, route.ts, [holiday_date]/, bulk-import/) ON DISK UNCOMMITTED on portal — needs validation gate + commit + push by W2 (post rate-limit reset).
