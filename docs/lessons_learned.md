...
### 2026-08-24: A skill generalized from your own recent work encodes your blind spots as rules

**What happened:** A `masterprompt` skill was written by generalizing from two worked examples — both authored by the same agent, the same week, in the same domain. It read as sound. Four review agents run in parallel (reuse / simplification / efficiency / altitude) found three defects that a self-review had missed entirely. The worst: the skill instructed authors to hand-copy boot order, evidence standards and halt conditions into every generated document — which `AGENT_TEMPLATE.md:255` explicitly forbids (*"Cite, do not duplicate. Authority docs are referenced by section, not pasted"*). The harm was already shipped: the masterprompt that seeded the skill had paraphrased `CLAUDE.md`'s six-layer ladder and rendered `rebuild_verifier()=0` as the weaker, non-runnable "integrity check passes", frozen into a document a receiving session reads at §0 — before it ever opens `CLAUDE.md`. Two further defects were in the skill's own advice: its stated success test ("can it execute without asking me anything") is satisfied by guessing, and a reconnaissance rule ("if you found only confirmations you did not look hard enough") made discovering a surprise a *pass condition*, which is how one gets invented.

**Why it was surprising:** the examples were good, and the extraction from them felt like abstraction. It was not — it was replication at a higher altitude. Every convention the author happened to follow became a universal, and every convention the author happened to violate became a rule for others to violate too. Self-review cannot catch this: the same blind spot that wrote the example approves the generalization. Worse, `docs/` already held **four** masterprompts, not two — the two unread ones (`docs/ceo/masterprompt-cos-v2.md`, Hebrew and research-shaped; `docs/plans/2026-07-24-production-picking-rollout.md`, a deliberately weaker executor, closed with a SHIPPED stamp) contradicted or extended the skill's universals in four separate places. The corrective evidence was on disk the whole time.

**Corrective:** before generalizing anything into a skill, doc, or template, **grep the repo for prior art of the same artifact type and read all of it — especially the instances you did not write.** `n=2, same author, same week` is not a sample; the variance you need lives in the examples that look least like yours. Then review the generalization from angles the author cannot occupy: reuse (does the repo already say this, or forbid it?), altitude (is this a principle or one workspace's habit?), efficiency, simplification. Parallel reviewers with distinct lenses disagreed with each other on two points here, and both disagreements were informative — a single reviewer would have returned one confident answer to each.

**Smaller trap from the same session:** in this container `~/.claude/skills/` is ephemeral and is reset from the account on restart, while the durable, version-controlled home for a workspace skill is the repo's own `.claude/skills/`. A skill written to the home directory works perfectly for the rest of the session and then silently ceases to exist. Write it to the repo; the loaded skill list picks it up from there just the same.
