...
### 2026-08-28: The rotated password is in two places, and the platform holds a third that is always right

**What happened:** Tom rotated the Postgres password in the Supabase dashboard around 12:00Z on 2026-08-27. Every scheduled job died within five minutes with `password authentication failed for user "postgres"` — `factory_os_jobs`, `sales-leads-poll`, `shopify_available_reconcile` on Supabase Edge, and the Railway Fastify API, all of which read one secret, `DATABASE_URL_POOLED`, stored independently in **Supabase Edge secrets** and **Railway variables**. It stayed down 19 hours. Four rounds of re-pasting the string, a full project restart, and a Railway redeploy all failed — because the password being pasted was never the one on the database.

**Why it was surprising:** the error names Postgres but never reaches it. `postgres_logs` carries **zero** authentication failures across the entire window; the rejection happens one layer earlier, in Supavisor (`ClientHandler: Exchange error`), which fetches the SCRAM verifier fresh on every attempt (`SecretChecker not started, using a one-off auth_query connection`). So the pooler is never the stale side — the client always is, and "the pooler cached it, wait or restart" is a dead end that cost two of the nineteen hours. The string also *looked* perfect: correct user `postgres.<project_ref>`, correct session-pooler host and port 5432, no whitespace, no quotes. Everything a human can check by eye was right.

**Corrective:**
1. **Do not debug a connection string by reading it.** A ~40-line read-only Edge Function that parses each candidate env var and attempts one `select 1` turns a five-minute cron guess-loop into a ten-second answer, and it can report `url_user`, `url_host`, length and whitespace **without ever echoing the password**. That probe is what ended this; it should have been step one, not hour six.
2. **`SUPABASE_DB_URL` is the ground truth.** The platform injects it into every Edge Function and keeps it current — it connected throughout the outage while the hand-maintained pooler string did not. It is the answer to "what is the password *right now*", and building the pooler string from it is what finally produced a working value. Any Edge Function that hard-depends on `DATABASE_URL_POOLED` should fall back to it (GAP-033).
3. **Length is a free discriminator.** The failing string carried a 10-character password, the working one 14. Comparing the env var's length against the password you believe you set catches a mis-paste in one call, before any deploy.
4. **Count the readers before rotating.** `DATABASE_URL_POOLED` has two independent copies that must move together, and `api/src/server.ts` reads `DATABASE_URL_POOLED ?? DATABASE_URL` — so a stale `DATABASE_URL_POOLED` silently beats a freshly-updated `DATABASE_URL`.

**Cost:** 19 hours. LionWheel poll, Shopify `available` reconcile, fulfillment bridge, freshness check, Green Invoice poll and the heartbeat all down; `order_intake.wa_event_log` stopped at 11:57Z; three Facebook leads failed to reach the database. The leads were recovered only because two earlier decisions happened to hold: the Make intake scenario carries a `Break` module that parks a failed run in the DLQ instead of dropping it, and `sales_core.lead` carries `UNIQUE (source, external_id)`, which made replaying all five queued items safe. Neither was designed for this incident; both saved it.

**Related:** the 2026-08-18 entry above covers the *other* Actions failure signature (`runner_id: 0`, 3-second failures on the private repo). The same signature recurred on 2026-08-24→27 from exhausted account Actions minutes, and the private-fails/public-passes split is still the discriminator.
