...
| 2026-08-23 | **Live intake + close-the-loop — BUILT, deployed dark.** Edge Function `sales-leads-poll` (v1, ACTIVE, `verify_jwt=true`): 10-minute Meta poll by time cursor, `backfill` over Meta's full 90-day retention, generic `ingest` route (second factor `X-Lead-Ingest-Token`), `probe` route that reports a real lead's FIELD NAMES before any mapping is trusted (§2.3 is the cautionary tale), Hebrew Resend alert with 🟢/🔁 framing, daily close-the-loop (`sales_core.convert_lead()` — the sole writer of `won`, from a Shopify order at-or-after the lead) and a heartbeat that sends **every day, healthy or not**. Migration 0328: `poll_run`, `lead_reject`, `convert_lead()`, `app_setting 'meta_poll'`, cron 27 (`*/10`) + 28 (`0 4 * * *`), vault-authenticated per 0309. Ships `enabled=false` so a token alone cannot start an unverified mapping; the heartbeat alarms on that gate so it cannot become a new silence. **Bring-up blocked on Tom's two secrets** (`META_PAGE_ACCESS_TOKEN`, `RESEND_API_KEY`) | gt-factory-os PR #226 · masterprompt: `docs/plans/2026-08-23-leads-live-intake-masterprompt.md` · spec §6/§7/§10: `docs/superpowers/specs/2026-08-10-sales-leads-pipeline-design.md` | node:test 49/49 (mapping 12, alerts 18, ingest flow 13 against a mocked Graph API + mocked Resend, conversion 6); pgTAP 22/22 on 0328 + **130/130 regression** on 0318–0327; root typecheck clean; `deno check`/`deno lint` clean; migration applied twice locally — idempotent. Deploy gates: stock-truth pre-flight **0 mismatches**, CI green, post-deploy health check observed — cron 27 fired 11:20:00Z, HTTP 200 `{"ok":true,"skipped":"missing META_PAGE_ACCESS_TOKEN — intake not yet live"}`, nothing written. Snapshot: `Sales-Machine/evidence/2026-08-23-live-intake-bringup.md` |
