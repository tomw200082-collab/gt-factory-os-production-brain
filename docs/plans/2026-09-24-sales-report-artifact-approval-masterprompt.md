...
**STATUS: ABANDONED — 2026-09-24.** Tom: the page must stay shared so the whole group sees
it. Under the §2.1 rule a shared page (public, or shared with latest version visible) asks
before every Routine republish, so the prompt cannot be removed while it is shared; moving
to the Team account does not change that (only a pinned version publishes silently).
Tom's replacement: a one-button approval email sent the moment the run parks —
`.claude/skills/weekly-sales-report/SKILL.md` step 5.9.
Evidence: D1 not attempted — `Artifact read` header 2026-09-24 still reads
`shared with anyone with the link (viewers see a pinned earlier version, not this live version)`;
routines.md re-fetched 2026-09-24, the four conditions unchanged; D2 not run.
Finding for Tom: the Routine's cron is `0 5 * * 1-5` (Mon–Fri 08:00 IL), not Sun–Thu.
