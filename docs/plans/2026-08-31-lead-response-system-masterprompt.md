...
**STATUS: PARTIALLY EXECUTED — 2026-08-31.** Not `SHIPPED`: four of the eight definition-of-done
conditions cannot be met by a session, and saying otherwise would break the standard this
document sets. What landed, and what did not:

| | Condition | Verdict |
|---|---|---|
| D1 | architecture decided and written | **RECOMMENDED, awaiting Tom** — `docs/decisions/2026-08-31-lead-intake-architecture.md` §7 |
| D2 | `leads_new` under 30 | **NOT MET** — still 141. All 141 have a phone; there is no data-hygiene path to 30. It needs people calling, or `U-011` capped. Costed instead: 7 working days for the recent tier, 10 for all |
| D3 | every lead has a non-`imported` event | **NOT MET** — 69 of 199 |
| D4 | answer bank live as data | **NOT DONE** — owned by the knowledge book, which is not in this repository yet |
| D5 | first-response time measured from real events | **MET** — `api_read.v_sales_funnel_metrics`, `v_sales_first_response` (gt-factory-os migration 0340). Median, 30-day window: **4,718 min ≈ 3.3 days** |
| D6 | kit send recorded on the lead | **HALF** — the `kit_sent` event type exists; no kit does |
| D7 | induced-silence alarm | **NOT TESTED** — the heartbeat is live and the pulse was green at 11:11Z, but no silence was induced |
| D8 | 36 tasks each carry a real status | **MET** — `docs/plans/2026-08-31-lead-setup-artifact-reconciliation.md` |

**The finding that changes the plan:** §2.3 below states GT has no WhatsApp Business API
account, no provider and no number in the API. All three are false — the Cloud API has
been live under Dualhook coexistence since **2026-06-26**, with 24,028 events logged.
Reframe 4's "two-to-three-week wait" is already served. Evidence:
`Sales-Machine/evidence/2026-08-31-lead-response-ground-truth.md` §6.

**Do not re-plan from §2.3 or §2.4 without reading that evidence file first.**
