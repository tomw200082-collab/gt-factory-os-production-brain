...
**STATUS: SHIPPED — executed 2026-08-30.**

> **Outcome.** System side complete as written. Storefront side executed under a
> narrowing Tom gave at approval time: archive only the products that no longer
> carry stock. 19 of 26 archived; 7 kept ACTIVE to sell through the remainder.
>
> - Migration: `gt-factory-os/db/migrations/0340_muza_line_retirement.sql` (applied prod)
> - Test: `gt-factory-os/db/tests/0340_muza_line_retirement.test.sql` (9 assertions)
> - Manifest: `gt-factory-os-production-brain/docs/analytics/2026-08-30_muza_retirement_manifest.json`
> - Record: `gt-factory-os-production-brain/docs/warehouses/catalog-truth.md`
> - State: `Sales-Machine/CURRENT_STATE.md` — opens U-014, U-015, U-016
>
> **Where the document was wrong, and it matters.** §3.2 and landmine 2 assume
> archiving Shopify first would start a five-minute reconciler error loop. It
> would not have. `shopify_available_reconcile/index.ts:49` filters its worklist
> on `i.status='ACTIVE'` *before* joining `integration_sku_map` (:63), and every
> MUZA item was already INACTIVE — so the reconciler was never syncing MUZA at
> all, and the quantities on those products are frozen leftovers. The mapping was
> still retired first, which is the right order regardless. Separately, retiring
> the 17 lionwheel rows has no runtime effect: the resolver
> (`api/src/integrations/lionwheel/sku_resolver.ts`) reads `source_channel` and
> `approval_status` only and never `mapping_status`, so sell-through still posts
> FG_OUT correctly.
>
> **D5 as written is not met, on purpose.** It asked for zero purchasable MUZA
> products. Tom's instruction at approval — `תשאיר מה שיש מלאי וכל השאר תארכב` —
> supersedes it. 7 products stay purchasable until their stock is gone.
>
> `AP-DRI-PSBLAP-1` (§2.4, §3.5) does not exist as a Shopify product or an
> `items` row. It lives only in sales history. Nothing to archive.
